 ...
 *        transmit buffer is empty, then writes data. After the last transmission, 
 *        the function waits for the BSY bit to reset before proceeding.
 */
spi_status_t
BSP_SPI_Transmit(SPI_TypeDef *p_spi, uint8_t *p_data, uint32_t size)
{
   ...
   * It is mandatory to wait first until TXE is set and then until BSY is cleared 
 ...
 *        that the SPI peripheral is not busy. Then it sends dummy data and waits 
 *        for the RXNE bit to set before reading the data register.
 */
spi_status_t
BSP_SPI_Receive(SPI_TypeDef *p_spi, uint8_t *p_data, uint32_t size)
{
  while (size)
  {
    ...
    // This indicates that the Rx buffer is not empty and data is ready to be read.
    while (!((p_spi->SR) & (1 << 0)))
    {
      if (u32_spi_timeout == 0)
      {
